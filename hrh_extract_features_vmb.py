...
class FeatureExtractor:
    ...
    def _check_feature_file_exist(self, file_name):
        file_base_name = os.path.basename(file_name)
        file_base_name = file_base_name.split(".")[0]
        info_file_base_name = file_base_name + "_info.npy"
        file_base_name = file_base_name + ".npy"
        if not os.path.exists(os.path.join(self.args.output_folder, file_base_name)):
            return False
        if not os.path.exists(os.path.join(self.args.output_folder, info_file_base_name)):
            return False
        return True
    ...
    def extract_features(self):
        ...
        ext = image_dir.split('.')[-1]
        is_img_file = os.path.isfile(image_dir) and ext in ['jpg', 'JPEG', 'gif', 'png']
        is_txt_file = os.path.isfile(image_dir) and ext in ['txt']
        if is_img_file:
            ...
            files_remain = [f for f in files if not self._check_feature_file_exist(f)]
            print('skipping %d existing files' % (len(files) - len(files_remain)))
            files = files_remain
